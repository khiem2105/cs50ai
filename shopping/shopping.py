...
def make_evidence(row):
    ...
    for i in range(17):
        if i in [0, 2, 4, 11, 12, 13, 14]:
            ...
            evidence.append(Month.get(row[i]))
        elif i == 15:
...
def evaluate(labels, predictions):
    ...
    """
    total_sensitivity = len([label for label in labels if label == 1])
    total_specificity = len([label for label in labels if label == 0])
    sensitivity = 0
    specificity = 0
    ...
    return (float(sensitivity/total_sensitivity), float(specificity/total_specificity))            
    # raise NotImplementedError
